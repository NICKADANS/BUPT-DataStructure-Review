...
23. (2015统考真题)用单链表保存m个整数，结点的结构为(data,link)，且|data|<=n(n为正整数)，现要求设计一个时间复杂度尽可能高效的算法，对于链表中data的绝对值相等的结点，仅保留第一次出现的结点而删除其余绝对值相等的结点。例如，若给定的单链表head为head->21->-15->-15->-7->15->^，则删除结点后的head为head->21->-15->-7->^
